...
// The formula is: min(baseDelay * 2^attempt, maxDelay) * (1 - jitter + rand * 2 * jitter)
func Delay(baseDelay, maxDelay time.Duration, jitter float64, attempt int) time.Duration {
	...
	// Apply jitter: multiply by factor in range [1-jitter, 1+jitter]
	if jitter > 0 {
		r := rand.NewMath()
...
// This includes network timeouts, connection refused, connection reset, etc.
func IsTransientError(err error) bool {
	...
	// Check for net.Error interface which has Timeout() and Temporary()
	if netErr, ok := err.(net.Error); ok {
		if netErr.Timeout() {
			return true
		}
		// Note: Temporary() is deprecated but still useful for some errors
		if netErr.Temporary() {
			return true
		}
	}
...
// FilterUsableAddresses filters addresses based on detected capabilities and config flags.
// It returns only addresses that match the available address families and are not disabled.
//
// Parameters:
//   - addresses: list of peer addresses to filter
//   - hasIPv4: whether this host has IPv4 capability
//   - hasIPv6: whether this host has IPv6 capability
//   - disableIPv4: config flag to disable IPv4
//   - disableIPv6: config flag to disable IPv6
func FilterUsableAddresses(addresses []string, hasIPv4, hasIPv6, disableIPv4, disableIPv6 bool) []string {
	...
	for _, addr := range addresses {
		...
		if ip == nil {
			// Not an IP address (hostname), include it if we have any capability
			if (hasIPv4 && !disableIPv4) || (hasIPv6 && !disableIPv6) {
...
// extractHostFromAddress extracts the host portion from an address.
// It handles URLs (tcp://host:port, quic://host:port), host:port, and plain host formats.
