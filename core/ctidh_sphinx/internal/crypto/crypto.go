...
var (
	// GroupElementLength is the length of a CTIDH group element in bytes.
	GroupElementLength = ctidh.PublicKeySize

	okmLength = MACKeyLength + StreamKeyLength + StreamIVLength + SPRPKeyLength + ctidh.PrivateKeySize
)
...
func KDF(ikm []byte) *PacketKeys {
	if len(ikm) != GroupElementLength {
		panic("KDF failure: ikm != GroupElementLength")
	}
	...
	k.BlindingFactor = make([]byte, ctidh.PrivateKeySize)
	copy(k.BlindingFactor, ptr[:ctidh.PrivateKeySize])
