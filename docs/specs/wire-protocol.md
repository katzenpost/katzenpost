...
\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*
\| \| Yawning Angel \| David Stainton \| \| Version 3 \| .. rubric::
Abstract
...
[\[KYBER\]](#KYBER){.citation} (NIST round 3). Kyber is one of the
finalists in the NIST post-quantum cryptography project. Please see the
Kyber project page for more information:
...
Framework [\[NOISE\]](#NOISE){.citation} along with \"Post Quantum
Noise\" paper [\[PQNOISE\]](#PQNOISE){.citation}. Older previous
versions of our transport were based on
[\[NOISEHFS\]](#NOISEHFS){.citation}.
...
[Kyber768X25519]{.title-ref} which is a hybrid KEM where the share
secret outputs of both X25519 and Kyber768 are combined.

Finally the [ChaChaPoly\_BLAKE2s]{.title-ref} parts of the protocol
...
long or short lived [Kyber768X25519]{.title-ref} keypair
([\[KYBER\]](#KYBER){.citation} and [\[RFC7748\]](#RFC7748){.citation}),
the public component of which is known to the other party in advance.
How such keys are distributed is beyond the scope of this document.
...
Implementations MUST preserve the Noise handshake hash ([h]{.title-ref})
for the purpose of implementing authentication (Section 2.3).
...
>
>     :   seconds since 1970-01-01 00:00:00 UTC for the responder.
...
with any of the information in the a`dditional_data` field such as the
user name, if any.
...
the unix\_time value to avoid leaking precise load information via
...
> -   The `ciphertext_length` field includes the Noise protocol overhead
>     of 16 bytes, for the Noise Transport message containing the
>     Ciphertext.
...
[KYBER]{#KYBER .citation-label}

:   Joppe Bos, Léo Ducas, Eike Kiltz, Tancrède Lepoint, Vadim
...
[NOISEHFS]{#NOISEHFS .citation-label}
...
[PQNOISE]{#PQNOISE .citation-label}

:   Yawning Angel, Benjamin Dowling, Andreas Hülsing, Peter Schwabe and
    Florian Weber, 2022, \"Post Quantum Noise\",
