...
// TestTimerQueueBasicOperations tests the timer queue priority queue operations
func TestTimerQueueBasicOperations(t *testing.T) {
	tq := NewTimerQueue()
	require.NotNil(t, tq)

	// Test empty queue
	require.Equal(t, 0, tq.Len(), "New queue should be empty")

	// Create test items with different priorities
	item1 := &TimerQueueItem{
		Priority: 100,
		Item:     "item1",
	}
	item2 := &TimerQueueItem{
		Priority: 50,
		Item:     "item2",
	}
	item3 := &TimerQueueItem{
		Priority: 150,
		Item:     "item3",
	}
	...
	// Peek should return lowest priority item (item2 with priority 50)
	peeked := tq.Peek()
	require.NotNil(t, peeked)
	require.Equal(t, int64(50), peeked.Priority)
	require.Equal(t, "item2", peeked.Item)
	require.Equal(t, 3, tq.Len(), "Peek should not remove item")

	// Pop should return items in priority order
	popped1 := tq.Pop()
	require.NotNil(t, popped1)
	require.Equal(t, int64(50), popped1.Priority)
	require.Equal(t, "item2", popped1.Item)
	require.Equal(t, 2, tq.Len())

	popped2 := tq.Pop()
	require.NotNil(t, popped2)
	require.Equal(t, int64(100), popped2.Priority)
	require.Equal(t, "item1", popped2.Item)
	require.Equal(t, 1, tq.Len())

	popped3 := tq.Pop()
	require.NotNil(t, popped3)
	require.Equal(t, int64(150), popped3.Priority)
	require.Equal(t, "item3", popped3.Item)
	require.Equal(t, 0, tq.Len())

	// Pop from empty queue should return nil
	popped4 := tq.Pop()
	require.Nil(t, popped4)

	t.Logf("✓ Timer queue operations work correctly")
}

// TestTimerQueueConcurrency tests concurrent access to timer queue
func TestTimerQueueConcurrency(t *testing.T) {
	tq := NewTimerQueue()
	tq.Start()
	defer tq.Halt()

	// Push multiple items concurrently
	const numItems = 100
	...
	require.Equal(t, numItems, tq.Len())
	...
	for i := 0; i < numItems; i++ {
		...
		require.GreaterOrEqual(t, item.Priority, lastPriority, "Items should be in priority order")
		lastPriority = item.Priority
	}

	require.Equal(t, 0, tq.Len())

	t.Logf("✓ Timer queue handles concurrent access correctly")
}
