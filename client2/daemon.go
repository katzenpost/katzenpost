...
func (d *Daemon) Start() error {
	...
	// NOTE(david): Our strategy here must be to send each of the
	// callback arge sets into a buffered channel where each callback
	// has a dedicated goroutine reading from it's cahnnel.
	d.cfg.Callbacks = &config.Callbacks{
		OnACKFn:      d.proxyReplies,
		OnConnFn:     d.listener.updateConnectionStatus,
		OnDocumentFn: d.onDocument,
	}
