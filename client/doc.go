...
thin client library to connect to a single client daemon which multiplexes
...
which gives them the capability to talk with the client daemon and in that way interact
with the mix network. The reason we call it a thin client library is because it does not
do any mixnet related cryptography since that is already handled by the client daemon.
