...
func (cfg *Config) validate() error {
	...
	if cfg.Authorities == nil || len(cfg.Authorities) == 0 {
		return fmt.Errorf("voting/client: Authorities is mandatory")
	}
	return nil
}
...
func (p *connector) initSession(ctx context.Context, linkKey kem.PrivateKey, signingKey sign.PublicKey, peer *config.Authority) (*connection, error) {
	...
	for i, idx := range idxs {
		...
		if err != nil {
			return nil, fmt.Errorf("%s: invalid address: %v", peerInfo(), err)
		}

		dialCtx, cancelFn := context.WithTimeout(ctx, dialTimeout)
		conn, err = dialFn(dialCtx, "tcp", u.Host)
		cancelFn()
		if err == nil {
			break
		}
...
func (c *Client) Post(ctx context.Context, epoch uint64, signingPrivateKey sign.PrivateKey, signingPublicKey sign.PublicKey, d *pki.MixDescriptor, loopstats *loops.LoopStats) error {
	...
	upload := &pki.SignedUpload{
		MixDescriptor: d,
	}
	if loopstats != nil {
		upload.LoopStats = loopstats
	}
	blob, err := upload.Marshal()
	if err != nil {
		return err
	}
	signed, err := cert.Sign(signingPrivateKey, signingPublicKey, blob, epoch)
	if err != nil {
		return err
	}
