...
func (c *Client) Post(ctx context.Context, epoch uint64, signingPrivateKey sign.PrivateKey, signingPublicKey sign.PublicKey, d *pki.MixDescriptor) error {
	...
	signed, err := pki.SignDescriptor(signingPrivateKey, signingPublicKey, d, epoch)
	if err != nil {
		return err
	}
...
func (c *Client) Verify(doc *pki.Document, epoch uint64) error {
	return pki.Verify(doc, c.verifiers, epoch)
}
