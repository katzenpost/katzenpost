...
- **2f. Note (no action now)**: `cache/go` is shared across distros; fine while builds are pure Go, scope per family if cgo (ctidh) returns.
...
- **3b. Allocate published host ports from the `base_port` sequence** (genconfig-derived, not user-specified): published band at `base_port+2000..+2004` (kpclientd, prometheus, grafana, pyroscope, kpclientd metrics), wired into compose publishes, `client.toml`/`thinclient.toml` dial addresses (currently hardcoded at `genconfig.go:1090–1091`), and prometheus scrape. In-bridge `kpclientd:64331` unchanged. Consequence (approved): default published ports move to 32000–32004, deliberately surfacing hardcoded ports — `genconfig_test.go`, `client/thin/thin_unit_test.go`, `CONFIG_CHANGES.md`, the kpclientd-metrics Makefile flag, and any external consumers (thin_client CI follows via the generated config).
- **3c. Pumba retargeting**: role-label based (line 227's regex won't match per-project names).
